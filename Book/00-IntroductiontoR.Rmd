...
#. Once you have reset your password, click on **Close** to close the shell and get back to (ref:RStudio). 
<!-- end enumerate -->
...
(ref:RStudio)  provides an integrated development environment (IDE) for (ref:R-package)  that makes (ref:R-package)  much easier to use.
It is freely available from [rstudio.com](http://rstudio.com) in versions for Macintosh, PC, or Linux. 
RStudio server provides access to (ref:RStudio)  via a web browser.  We will generally assume that (ref:RStudio) 
is being used throughout. Although most things can be done without (ref:RStudio)  as well, our descriptions
may apply only to (ref:RStudio) .


### Loading packages

(ref:R-package)  is divided up into packages.  A few of these are loaded every time you run (ref:R-package)  but most have to be selected.  
This way you only have as much of (ref:R-package)  as you need.

In the **Packages** tab in (ref:RStudio) , 
...
Notice that (ref:RStudio)  divides its world into four panels.  Several of the panels are further subdivided into multiple tabs. 
The console panel is where we type commands that (ref:R-package)  will execute. 

(ref:R-package)  can be used as a calculator.  Try typing the following commands in the console panel.
```{r eval=TRUE} 
...
### The (ref:RStudio)  help system

There are several ways to get (ref:RStudio)  to help you when you forget something. Most objects in packages have help files that you can access by typing something like:
```r
?bargraph
?histogram
?HELPrct
...
### Error messages
When things go wrong, (ref:R-package)  tries to help you out by providing an error message. If you can't make sense of the message, you can try copying and pasting your command and the error message and sending to me in an email.  One common error message is illustrated below.
...
Computers are great for doing complicated computations quickly, but you have to speak to them on their terms.  Here are few things that will help you communicate with (ref:R-package) 
<!-- begin enumerate -->

(@) (ref:R-package)  is case-sensitive

    If you mis-capitalize something in (ref:R-package)  it won't do what you want.

(@) 
Functions in (ref:R-package)  use the following syntax:
...
(@) TAB completion and arrows can improve typing speed and accuracy.
    ...
    If you begin a command and hit the TAB key, (ref:R-package)  will show you a list of possible ways to complete the command.  If you hit TAB after the opening parenthesis of a function, it will show you the list of arguments it expects. The up and down arrows can be used to retrieve past commands.
...
## Data in (ref:R-package) 

### Data in Packages
Most often, data sets in (ref:R-package)  are stored in a structure called a **data frame**.  There are a number of data sets built into (ref:R-package)  and many more that come in various add on packages.  The **`Lock5withR`** package, for example, contains all the data sets from our text book. In the book, data set names are printed in bold text.
...
So far we have been using data that lives in (ref:R-package)  packages.  This has allowed us
...
data into (ref:R-package)  and have some tools for manipulating the data once it is there.  
...
errors).  This section describes how to get data from a spreadsheet into (ref:R-package) 

#### While you are still in the spreadsheet

If you are creating your own data in a spreadsheet with the intent of bringing
into (ref:R-package)  (or some other statistical package) for analysis, it is important that
...
1. The first row should contain variables names.
    ...
    These should be names that will work well in (ref:R-package)   This usually means they will be relatively short and avoid spaces and punctuation.
...
The comma separated values (csv) format has become a standard way of transferring data between programs. Both Google and Excel can export to this format, and (ref:R-package)  can import from this format.  Once your dataare ready to go, export them to csv.  Give the file a good name, and remember where you have put it.

#### Uploading the data ((ref:RStudio)  server only)

To get the data from your computer onto the server, you need to **upload** the data.  (You can skip this step if you are working with a local copy of (ref:RStudio) )  Uploading transfers a copy of your data from your computer onto the server (the "cloud").  This is like uploading pictures to Facebook so you can later use them in posts or as a cover photo or tag your friends or whatever else once the photo is on Facebook.
...
Now that the file is on the server, you can import it into (ref:R-package)   This takes place in the **Environment** tab.  Once there, choose **Import Dataset** and then **From Text File...**.  
<!-- begin center -->
...
The data set should now be ready for use in (ref:R-package) 
...
Even if you primarily use the (ref:RStudio)  interface to import data, it is good to know about the command line methods since these are required to import data into scripts, RMarkdown, and Rnw files.  CSV files (and a few other types of files as well) can be read with 
...
because SAS uses a period (.) to code missing data, and some csv exporters use ``-`'.  By default (ref:R-package)  reads these as string data, which forces the entire variable to be of character type instead of numeric.


#### Importing Other Kinds of Data

Many (ref:R-package)  packages provide the ability to load data from special data files.  If you have data in some other format, there may well be
...
Most of what we will do in this chapter makes use of a single (ref:R-package)  template:
...
1. What do you want (ref:R-package)  to do?
    * this determines what function to use (goal).
    
1. What must (ref:R-package)  know to do that?
...
1. What does (ref:R-package)  need to know to do this?
...
Generally, it is a good idea to keep raw data (like <span style="color:green">Sepal.Length</span> and <span style="color:green">Sepal.Width</span> in your data file, but let (ref:R-package)  do the computation of derived variables for you.  Among other advantages, if you ever fix an error in a <span style="color:green">Sepal.Length</span> measurement, you don't have to worry about remembering to also recompute the ratio.  Futhermore, your (ref:R-package)  code documents how the derived value was computed.

### Saving Data

`write.csv()()` can be used to save data from (ref:R-package)  into csv formatted files. This can be useful for exporting to some other program.
<!-- Relies on data from a broken link -->
<!-- ```{r writingData} -->
...
Data can also be saved in native (ref:R-package)  format.  Saving data sets (and other (ref:R-package)  objects) using `save()()` has some advantages over other file formats:
<!-- begin itemize -->
...
<!-- end itemize -->
The downside is that these files are only readable in (ref:R-package) 
<!-- Relies on data from a broken link -->
...
Although you can export plots from (ref:RStudio)  for use in other applications, there is another way of preparing documents that has many advantages.  
(ref:RStudio)  provides several ways to create documents that include text, (ref:R-package)  code, (ref:R-package)  output, graphics, even mathematical notation all in one document.  The simplest of these is R Markdown.  
...
The second type of section is an (ref:R-package)  code chunk.  These are colored differently to make them easier to see.  You can insert a new code chunk by selecting "Insert Chunk" from the "Chunks" 
menu:
<!-- begin center -->

  ![Caption](images/InsertChunk.png)
<!-- end center -->
(You can also type ```{r}! to begin and ```! to end the code chunk if you would rather type.) You can put any (ref:R-package)  code in these code chunks and the results (text output or graphics) as well as the (ref:R-package)  code will be displayed in your HTML file.

There are options to do things like (a) run (ref:R-package)  code without displaying it, (b) run (ref:R-package)  code without
...
The **`mosaic`** package provides some templates that are available if you choose "from template" when creating an RMarkdown file in (ref:RStudio) .
...
(ref:RStudio)  makes it easy to generate HTML, PDF, or Word documents from your RMarkdown.  Just remember that if you edit any of these files after you generate them 
...
<!-- 	(W.A. Wallis) -->
  ...
  #. a collection of procedures and principles for gaining information in
	...
	uncertainty (J. Utts @Utts:2005:SeeingThroughStats),

  #. a way of taming uncertainty, of turning raw data into arguments that
	can resolve profound questions (T. Amabile @AgainstAllOdds:video),
<!--   #.  the science of gaining information from numerical data (D.S. Moore) -->
 ...
 #. the mathematics of the collection, organization, and interpretation of
   ...
   (American Heritage Dictionary @AmHerDictionary:1982).
<!-- end itemize -->
...
So here's the plan.  We'll flip 10 coins.  We'll call the heads correct guesses and the tails incorrect guesses.  Then we'll flip 10 more coins, and 10 more, and 10 more, and ...  That would get pretty tedious. Fortunately, computers are good at tedious things, so we'll let the computer do the flipping for us using a tool in the **`mosaic`** package. This package is already installed in our (ref:RStudio)  server.  If you are running your own installation of (ref:R-package)  you can install **`mosaic`** using the following command:
...
Typing `rflip(10)` a bunch of times is almost as tedious as flipping all those coins.   But it is not too hard to tell (ref:R-package)  to `do()()` this a bunch of times.
```{r flip2, cache=TRUE}
do(2) * rflip(10)
```
...
Let's get (ref:R-package)  to `do()()` it for us 10,000 times and make a table of the results.
